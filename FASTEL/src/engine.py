...
from . import losses
from . import models_multitask
from . import utils_multitask
from . import layers
from . import soft_trees
...
class MultiTaskTrees:
    ...
    def create_model(self):
        ...
        if self.loss_criteria in ["mse", "poisson"]:
            ...
            for i, rp in enumerate(outputs):
                ...
                if self.loss_criteria == "mse":
                    ypred = tfp.layers.DistributionLambda(
                        lambda t: tfp.distributions.Normal(t[..., 0], scale=1.0),
                        name="task{}".format(i),
                    )(ypred)
                    # ypred = tf.keras.layers.Lambda(
                    #     lambda t: t, name="task{}".format(i)
                    # )(ypred)
                elif self.loss_criteria == "poisson":
                    ypred = tfp.layers.DistributionLambda(
            ...
            if self.architecture == "shared":
                ...
                x = submodel.input
                outputs = submodel(x)
                regression_preds, classification_logodds = tf.split(
                    outputs, num_or_size_splits=2, axis=1
                )
                regression_preds = tf.keras.layers.Activation(
                    "linear", name="MultitaskRegression"
                )(regression_preds)
                classification_logodds = tf.keras.layers.Activation(
                    "linear", name="MultitaskClassification"
                )(classification_logodds)
                regression_preds = tf.split(
                    regression_preds, num_or_size_splits=self.num_tasks, axis=1
                )
                classification_logodds = tf.split(
                    classification_logodds, num_or_size_splits=self.num_tasks, axis=1
                )
                ...
                outputs = submodel(x)
                task_preds = tf.split(
                    outputs, num_or_size_splits=self.num_tasks, axis=1
                )
                regression_preds = []
                classification_logodds = []
                for i, task_pred in enumerate(task_preds):
                    regression_pred, classification_logodd = tf.split(
                        task_pred, num_or_size_splits=2, axis=1
                    )
                    regression_pred = tf.keras.layers.Activation(
                        "linear", name="MultitaskRegression-task{}".format(i)
                    )(regression_pred)
                    classification_logodd = tf.keras.layers.Activation(
                        "linear", name="MultitaskClassification-task{}".format(i)
                    )(classification_logodd)
            ...
            for i, (rp, cp) in enumerate(zip(regression_preds, classification_logodds)):
                ...
                ypred = tfp.layers.DistributionLambda(
                    layers.zero_inflation_mixture_singletask, name="task{}".format(i)
                )(ypred)
                ypreds.append(ypred)
            ...
            if self.architecture == "shared":
                ...
                x = submodel.input
                outputs = submodel(x)
                regression_preds, dispersion_preds = tf.split(
                    outputs, num_or_size_splits=2, axis=1
                )
                regression_preds = tf.keras.layers.Activation(
                    "linear", name="MultitaskRegression"
                )(regression_preds)
                dispersion_preds = tf.keras.layers.Activation(
                    "linear", name="MultitaskDispersion"
                )(dispersion_preds)
                regression_preds = tf.split(
                    regression_preds, num_or_size_splits=self.num_tasks, axis=1
                )
                dispersion_preds = tf.split(
                    dispersion_preds, num_or_size_splits=self.num_tasks, axis=1
                )
                ypreds = []
                for i, (rp, dp) in enumerate(zip(regression_preds, dispersion_preds)):
                    ...
                    ypred = tfp.layers.DistributionLambda(
                        lambda t: tfp.distributions.NegativeBinomial.experimental_from_mean_dispersion(
                            mean=tf.math.exp(t[..., 0]),
                            dispersion=tf.math.exp(t[..., 1]),
                        ),
                        name="task{}".format(i),
                    )(ypred)
                    ypreds.append(ypred)
        ...
        # model.summary()
        model.compile(loss=self.loss, optimizer=self.optimizer, weighted_metrics=[])
        return model

    def process_data(self, y_true, w):
    ...
    def train(
        ...
        if len(get_available_gpus()) == 0:
            history = self.model.fit(
                x=x_train,
                y=tf.split(
                    y_train_processed, num_or_size_splits=self.num_tasks, axis=1
                ),
                ...
                validation_data=(
                    x_valid,
                    tf.split(
                        y_valid_processed, num_or_size_splits=self.num_tasks, axis=1
                    ),
                    w_valid_dict,
                ),
                verbose=0,
    ...
    def predict(self, x):
        ...
        # y_pred = tf.stack(
        #     [yv.mean() for yv in y_pred_dist], axis=1
        # ).numpy()  # compute expectation
    ...
    def compute_metrics(
        ...
        y_pred_dist = self.predict_distribution(x)
        negloglikelihood = losses.NegativeLogLikelihood()(
            y_true_processed,
            y_pred_dist,
            w_masked,
        ).numpy()
        ...
        poisson_deviance, _, _ = utils_multitask.multitask_masked_poisson_deviance(
            y_true, y_pred, w
        )
        metrics = {
